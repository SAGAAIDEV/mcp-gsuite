...
async def query_gmail_emails(
    ...
    Args:
        user_id: The EMAIL of the Google account for this action.
                 Ensure this email is configured in the application.
    ...
    """
    try:
        await asyncio.to_thread(gauth.setup_oauth2, user_id)
        ...
        # logger.exception(f"Error in query_gmail_emails for {user_id}")
        error_message = {"error": f"Failed to query emails for {user_id}: {str(e)}"}
        return json.dumps(error_message, indent=2)


@mcp.tool()
async def get_gmail_email(user_id: str, email_id: str) -> str:
    """Retrieves a complete Gmail email message by its ID.
    Includes the full message body and metadata, including attachment IDs.
    ...
    try:
        ...
        email_data_tuple = await asyncio.to_thread(
            gmail_service.get_email_by_id_with_attachments, email_id
        )

        # Assuming get_email_by_id_with_attachments returns (email_details, attachments_list)
        # or (None, None) on failure.
        if email_data_tuple and email_data_tuple[0] is not None:
            email_details, attachments_list = email_data_tuple
            email_details["attachments"] = (
                attachments_list  # Merge attachments into the main dict
            )
            return json.dumps(email_details, indent=2)
        else:
            # Corrected json.dumps string
            return json.dumps(
                {
                    "error": f"Failed to retrieve email with ID: {email_id}",
                    "user_id": user_id,
                },
                indent=2,
            )
        ...
        # import logging
        # logger = logging.getLogger(__name__)
        # logger.exception(f"Error in get_gmail_email for {user_id} with email_id {email_id}")
        # Corrected json.dumps string
        error_message = {
            "error": f"Failed to retrieve email {email_id} for {user_id}: {str(e)}"
        }
        return json.dumps(error_message, indent=2)
